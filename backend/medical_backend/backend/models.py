...
class LabTest(models.Model):
    ...
    def get_default_parameters(cls, test_code, gender=''):
        test = cls.objects.filter(code=test_code).first()
        if not test:
            return []
        ...
        for param in test.parameters:
            ...
            if gender == 'Male' and 'normalRangeMale' in param_obj:
                param_obj['normalRange'] = param_obj['normalRangeMale']
            elif gender == 'Female' and 'normalRangeFemale' in param_obj:
                param_obj['normalRange'] = param_obj['normalRangeFemale']
            
            default_params.append({
                'name': param_obj['name'],
                'value': '',
                'unit': param_obj['unit'],
                'normalRange': param_obj['normalRange'],
                'flag': ''
            })
    ...
    def create_seed_data(cls):
        ...
        for test_data in seed_data:
            test = cls(code=test_data['code'], name=test_data['name'], category=test_data['category'],
                      turnaroundTime=test_data['turnaroundTime'], sampleType=test_data['sampleType'],
                      description=test_data['description'], parameters=test_data['parameters'])
            test.save()
...
class Patient(models.Model):
    ...
    def generate_patient_id(self):
        ...
        self.patientId = f'LCL-{year}-{str(count).zfill(4)}'
    
    def update_status(self):
        completed = sum(1 for test in self.tests if test.get('status') == 'Completed')
        in_progress = sum(1 for test in self.tests if test.get('status') in ['In Progress', 'Completed'])
        
        if completed > 0 and len(self.tests) > 0:
        ...
        if completed == len(self.tests) and completed > 0:
            self.dateCompleted = timezone.now()
        
        self.save(update_fields=['status', 'dateCompleted'])
...
class PatientTest(models.Model):
    ...
    completedBy = models.CharField(max_length=50, null=True, blank=True)
    parameters = models.JSONField(default=list)
    ...
    def __str__(self):
        return f"{self.patient.name} - {self.testName}"


class TestResultParameter(models.Model):
    id = models.AutoField(primary_key=True)
    test = models.ForeignKey(PatientTest, on_delete=models.CASCADE, related_name='parameters')
    name = models.CharField(max_length=100)
    value = models.CharField(max_length=50)
    unit = models.CharField(max_length=50)
    normalRange = models.CharField(max_length=100)
    flag = models.CharField(max_length=20, default='')
    
    @classmethod
    def calculate_flag(cls, value, normal_range):
        ...
        try:
            ...
            lr = normal_range.lower().strip()
            
            if lr in ('negative', 'non-reactive') or 'negative' in lr or 'non-reactive' in lr:
                if 'positive' in lv or 'reactive' in lv:
                    return 'Positive'
                if 'negative' in lv or 'non-reactive' in lv:
                    return 'Negative'
                return 'Abnormal'
        ...
        dash_match = re.match(r'(\d+\.?\d*)\s*[–-]\s*(\d+\.?\d*)', normal_range.strip())
        if dash_match:
        ...
        gt_match = re.match(r'[>≥]\s*(\d+\.?\d*)', normal_range.strip())
        if gt_match:
            min_val = float(gt_match.group(1))
            return 'Low' if num_value < min_val else 'Normal'
        
        lt_match = re.match(r'[<≤]\s*(\d+\.?\d*)', normal_range.strip())
        if lt_match:
            max_val = float(lt_match.group(1))
            return 'High' if num_value > max_val else 'Normal'
...
class Patient(models.Model):
    ...
    def get_stats(self):
        ...
        return {
            'total': len(Patient.objects.all()),
            'pending': len(Patient.objects.filter(status='Pending')),
            'inProgress': len(Patient.objects.filter(status='In Progress')),
            'completed': len(Patient.objects.filter(status='Completed')),
            'todayCount': len(Patient.objects.filter(dateRegistered__date=today)),
            'urgent': len(Patient.objects.filter(priority='Urgent')),
        }
...
class Patient(models.Model):
    ...
    def update_status(self):
        from django.db.models import Count
        completed = self.tests.filter(status='Completed').count()
        in_progress = self.tests.filter(status__in=['In Progress', 'Completed']).count()
        
        self.status = 'Completed' if completed > 0 and self.tests.count() > 0 else 'Pending'
        if in_progress > 0:
            self.status = 'In Progress'
        
        if completed == self.tests.count() and completed > 0:
            self.dateCompleted = timezone.now()
...
class TestResultParameter(models.Model):
    ...
    def calculate_flag(cls, value, normal_range):
        ...
        dash_match = normal_range.strip().match(r'(\d+\.?\d*)\s*[–-]\s*(\d+\.?\d*)')
        if dash_match:
            min_val = float(dash_match.group(1))
            max_val = float(dash_match.group(2))
            if num_value < min_val:
        ...
        gt_match = normal_range.strip().match(r'[>≥]\s*(\d+\.?\d*)')
        if gt_match:
            min_val = float(gt_match.group(1))
            return 'Low' if num_value < min_val else 'Normal'
        
        lt_match = normal_range.strip().match(r'[<≤]\s*(\d+\.?\d*)')
        if lt_match:
            max_val = float(lt_match.group(1))
            return 'High' if num_value > max_val else 'Normal'
